...
var Command = function () {
  ...
  _createClass(Command, [{
    ...
    value: function runValidations() {
      ...
      if (!_FileSystemUtilities2.default.existsSync(this.repository.asiniJsonLocation)) {
      ...
      if (this.flags.independent && !this.repository.isIndependent()) {
        this.logger.warn("You ran asini with `--independent` or `-i`, but the repository is not set to independent mode. " + "To use independent mode you need to set your `asini.json` \"version\" to \"independent\". " + "Then you won't need to pass the `--independent` or `-i` flags.");
        this._complete(null, 1);
        return;
      }

      if (process.env.NODE_ENV !== "test" && this.asiniVersion !== this.repository.asiniVersion) {
        this.logger.warn("Asini version mismatch: The current version of asini is " + this.asiniVersion + ", " + ("but the Asini version in `asini.json` is " + this.repository.asiniVersion + ". ") + ("You can either run `asini init` again or install `asini@" + this.repository.asiniVersion + "`."));
        this._complete(null, 1);
        return;
      }
